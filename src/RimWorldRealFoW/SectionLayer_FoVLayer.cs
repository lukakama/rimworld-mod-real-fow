...
namespace RimWorldRealFoW {
	...
	public class SectionLayer_FoVLayer : SectionLayer {
		...
		private int prefFadeSlowMult = 4;
		...
		public override void Regenerate() {
			...
			if (pawnFog != null) {
				...
				long fogTransitionTick = Find.TickManager.TicksGame * prefFadeSlowMult;
					...
					for (int z = cellRect.minZ; z <= cellRect.maxZ; z++) {
						...
						for (int m = 0; m < 9; m++) {
							...
							if (!prefEnableFade || firstGeneration) {
								if (firstGeneration) {
									subMesh.colors.Add(new Color32(255, 255, 255, alpha));
								} else if (subMesh.colors[colorIdx].a != alpha) {
									subMesh.colors[colorIdx] = new Color32(255, 255, 255, alpha);
								}
								if (prefEnableFade) {
		...
		public override void DrawLayer() {
			if (prefEnableFade && this.Visible && activeFogTransitions) {
				...
				List<Color32> colors = subMesh.colors;
